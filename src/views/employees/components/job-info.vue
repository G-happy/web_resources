...
import { getJobDetailAPI, getEmployeesSimpleAPI, updateJobAPI } from '@/api'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async getPersonalDetail() {
      try {
        this.formData = await getJobDetailAPI(this.userId) // 获取员工数据
      } catch (error) {
        return error
      }
    },
    ...
    async getEmployeesSimple() {
      try {
        this.depts = await getEmployeesSimpleAPI()
      } catch (error) {
        return error
      }
    },
